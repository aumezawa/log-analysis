...
import { Clock, Cpu, Diagram3, Diagram3Fill, Display, FileEarmarkText, Gear, Grid3x3Gap, HddNetwork, HddStack, JournalCheck, Server, Tag, Tags } from "react-bootstrap-icons"
...
const OPTIONS = ["All", "System", "Hardware", "Network", "SCSI & Disk", "Software & Driver"]
...
const HostInfoBox = React.memo<HostInfoBoxProps>(({
  ...
  const render = () => {
    ...
    if (["All", "Hardware"].includes(data.current.display)) {
      tables.push(
        <Table
          key="hardware"
    ...
    if (["All", "Hardware", "SCSI & Disk"].includes(data.current.display)) {
    ...
    if (["All", "Software & Driver"].includes(data.current.display)) {
  ...
  return (
    <div className={ className }>
      ...
      { !status.current.progress && !!data.current.hosts.length &&
        <LayerFrame
          head={
            <SelectForm
              className="my-2"

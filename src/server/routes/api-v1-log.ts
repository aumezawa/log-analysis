...
router.route("/:domain/projects/:projectName/bundles/:bundleId/vms/:vmName/vmx")
.get((req: Request, res: Response, next: NextFunction) => {
  return Project.getVmVmxPath(req.token.usr, req.domain, req.project, req.bundleId, req.params.vmName)
  .then((vmVmxPath: string) => {
    // OK
    return res.status(200).json({
      msg: "You get a virtual machine vmx path.",
      vmxpath: vmVmxPath
    })
  })
  .catch((err: any) => {

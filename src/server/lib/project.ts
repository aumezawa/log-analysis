...
function decompressBundleSync(file: string, option?: string, preserve: boolean = false): Array<FileInfo> {
  if (option === "vmtools") {
    const filesInfo = Vmtools.decompressBundleSync(file, preserve)
    if (filesInfo) {
...
function decompressBundle(file: string, option?: string, preserve: boolean = false): Promise<Array<FileInfo>> {
  return new Promise<Array<FileInfo>>((resolve: (filesInfo: Array<FileInfo>) => void, reject: (err? :any) => void) => {
    return setImmediate(() => {
      let err = new Error(`Resource: ${ file } cloudn't be decompressed. It may be unsupported file type...`)
      err.name = "External"
      const filesInfo = decompressBundleSync(file, option, preserve)
      return filesInfo ? resolve(filesInfo) : reject(err)
    })
  })
}
...
export function registerBundleResource(user: string, domain: string, project: string, bundleTgz: string, description: string = "", preserve: boolean = false): Promise<BundleInfo> {
  return new Promise<BundleInfo>((resolve: (bundle: BundleInfo) => void, reject: (err?: any) => void) => {
    return setImmediate(() => {
      ...
      .then(() => {
        return decompressBundle(bundlePath, "vmtools", preserve)
      })
...
export function getVmVmxPath(user: string, domain: string, project: string, bundleId: string, vm: string): Promise<string> {
  return new Promise<string>((resolve: (vmVmxPath: string) => void, reject: (err?: any) => void) => {
    return setImmediate(() => {
      let err = new Error(`vmtools: An internal error occurred.`)
      err.name = "Internal"
      const vmVmxPath = Vmtools.getVmVmxPathSync(getBundleResourcePathSync(user, domain, project, bundleId), vm)
      return vmVmxPath ? resolve(vmVmxPath) : reject(err)
    })
  })
}
